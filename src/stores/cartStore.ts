...
export const useCartStore = create<CartStore>((set, get) => ({
  ...
  removeItem: (item: Item) => {
    return set((state) => {
      ...
      const newCart = { 
        ...state.cart, 
        pendingItems: state.cart.pendingItems.filter(i => i.product.id !== item.product.id), 
        confirmedItems: state.cart.confirmedItems.filter(i => i.product.id !== item.product.id)
      };
      console.log('Updated cart state:', newCart);
      return { cart: newCart }; 
    });
  ...
  cancelDelivery: () => set((state) => {
    if (!state.cart) {
      throw new Error('Cart is not initialized');
    }
    return { cart: { ...state.cart, nextDelivery: undefined, confirmed: false } };
  }),
  isCartValid: () => {
    const state = get();
